...
if not GEMINI_API_KEY:
    st.error("🔴 Gemini API Key not found. Please ensure GEMINI_API_KEY is set in your .env file.")
    st.stop()
...
# --- Updated call_gemini Function for Google Gemini API ---
def call_gemini(history):
    safe_history = []
    for msg in history:
        if isinstance(msg, dict) and "role" in msg and "content" in msg:
            # Gemini API expects "user" and "model" roles instead of "assistant"
            role = "model" if msg["role"] == "assistant" else "user"
            safe_history.append({"role": role, "parts": [{"text": str(msg["content"])}]})
    ...
    **ESCAPING QUOTES:**
    ...
    messages = [
        ...
        {"role": "model", "parts": [{"text": "[{\"action\": \"chat\", \"content\": \"Okay, I understand the strict JSON formatting rules (double quotes, escaping) and the need to provide full file content on updates. I will respond only with the valid JSON array. Ready.\"}]"}]}
    ]
    ...
    try:
        ...
        data = {
            ...
            "generationConfig": {
                "temperature": 0.5,
                "maxOutputTokens": 8000  # Increased token limit to handle larger responses
            }
        }
        response = requests.post(url, headers=headers, json=data)
        if response.status_code != 200:
            if response.status_code == 429:
                st.error("🔴 Gemini API Quota/Rate Limit Exceeded.")
            elif response.status_code == 401 or response.status_code == 403:
                st.error("🔴 Gemini API call failed: Invalid API Key or Permissions Issue.")
            else:
                st.error(f"🔴 Gemini API call failed with status {response.status_code}: {response.text}")
        ...
        if 'candidates' in response_json and len(response_json['candidates']) > 0:
            if 'content' in response_json['candidates'][0] and 'parts' in response_json['candidates'][0]['content']:
                # Extracting the response text from Gemini API structure
            ...
            st.error("🔴 Empty or invalid Gemini API response.")
